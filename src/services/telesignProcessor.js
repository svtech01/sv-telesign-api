...
export async function processCSV(filePath, options = {}) {
  ...
  try {
    ...
    if (data.length < thresholdForBatching && batchSize == "all") {
      ...
      if (mode == "test") {
        let processor = await processCSVAllTest(data, { ...options, concurrency });
        if (processor) {
...
export async function processCSVAllTest(data, options) {
  try {
      ...
      data.map((row) =>
        limit(async () => {
          try {
            ...
            const result = await validatePhoneRowTest(sanitizedRow, liveStatus);
            results.push(result);
...
export async function validatePhoneRowTest(row, includeLiveStatus = false) {
  let phoneE164 =
    (row.phone || row.e164_format || row.phone_e164 || row.phone_number || "").trim();
